...
export async function onRequestPost({ env, request }) {
  ...
  ).bind(USER_ID, body.name, body.icon || '📋', JSON.stringify(body.fields || [])).run()
...
export async function onRequestPut({ env, request }) {
  ...
  ).bind(body.name, body.icon || '📋', JSON.stringify(body.fields || []), body.id, USER_ID).run()
...
export async function onRequestDelete({ env, request }) {
  ...
  await env.DB.prepare('DELETE FROM custom_apps WHERE id = ? AND user_id = ?').bind(id, USER_ID).run()
  return json({ success: true })
}
