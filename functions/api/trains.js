...
export async function onRequestPost({ env, request }) {
  ...
  ).bind(USER_ID, body.train, body.direction, body.boardStation || body.board_station, body.exitStation || body.exit_station || '', JSON.stringify(body.days || []), body.note || '').run()
...
export async function onRequestPut({ env, request }) {
  ...
  ).bind(body.train, body.direction, body.boardStation || body.board_station, body.exitStation || body.exit_station || '', JSON.stringify(body.days || []), body.note || '', body.id, USER_ID).run()
...
export async function onRequestDelete({ env, request }) {
  ...
  await env.DB.prepare('DELETE FROM train_schedule WHERE id = ? AND user_id = ?').bind(id, USER_ID).run()
  return json({ success: true })
}
