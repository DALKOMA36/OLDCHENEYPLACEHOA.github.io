...
export async function onRequestPost({ env, request }) {
  ...
  ).bind(USER_ID, body.title, body.priority || 'medium', body.assignee || '', body.dueDate || body.due_date || null, body.recurring ? 1 : 0, body.category || 'personal', body.completed ? 1 : 0).run()
...
export async function onRequestPut({ env, request }) {
  ...
  ).bind(body.title, body.priority || 'medium', body.assignee || '', body.dueDate || body.due_date || null, body.recurring ? 1 : 0, body.category || 'personal', body.completed ? 1 : 0, body.id, USER_ID).run()
...
export async function onRequestDelete({ env, request }) {
  ...
  await env.DB.prepare('DELETE FROM tasks WHERE id = ? AND user_id = ?').bind(id, USER_ID).run()
  return json({ success: true })
}
